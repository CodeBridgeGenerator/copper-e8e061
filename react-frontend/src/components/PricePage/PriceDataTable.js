...
const PriceDataTable = ({ items, onEditRow, onRowDelete, onRowClick }) => {
    const dt = useRef(null);
    const urlParams = useParams();
    const [showUpload, setShowUpload] =useState(false)

const currencyTemplate0 = (rowData, { rowIndex }) => <InputNumber value={rowData.MYR}  mode="currency" currency="MYR" locale="en-US" disabled={true} />
...
<Column field="MYR" header="MYR" body={currencyTemplate0} sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload Price Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>
        </>
    );
