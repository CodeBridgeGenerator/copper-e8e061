...
import priceFakerFactory from "./priceFakerFactory";
...
const PriceFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = priceFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("price").create(elem));
...
export default connect(mapState, mapDispatch)(PriceFakerDialogComponent);
