...
const FundnameDataTable = ({ items, onEditRow, onRowDelete, onRowClick }) => {
    const dt = useRef(null);
    const urlParams = useParams();
    const [showUpload, setShowUpload] =useState(false)

const pTemplate0 = (rowData, { rowIndex }) => <p >{rowData.Fund}</p>
const calendarTemplate1 = (rowData, { rowIndex }) => <p>{new Date(rowData.Inception).toLocaleDateString()}</p>
const pTemplate2 = (rowData, { rowIndex }) => <p >{rowData.Manager}</p>
...
<Column field="Fund" header="Fund Name" body={pTemplate0} sortable style={{ minWidth: "8rem" }} />
<Column field="Inception" header="Inception" body={calendarTemplate1} sortable style={{ minWidth: "8rem" }} />
<Column field="Manager" header="Manager" body={pTemplate2} sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload Fundname Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>
        </>
    );
...
export default FundnameDataTable;