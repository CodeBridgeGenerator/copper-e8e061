...
import fundnameFakerFactory from "./fundnameFakerFactory";
...
const FundnameFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = fundnameFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("fundname").create(elem));
...
export default connect(mapState, mapDispatch)(FundnameFakerDialogComponent);
